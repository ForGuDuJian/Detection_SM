    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = validate.run(data_dict, batch_size=batch_size // WORLD_SIZE * 2, imgsz=imgsz,
                                                single_cls=single_cls, half=amp, model=ema.ema, dataloader=val_loader,
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = validate.run(data_dict, batch_size=batch_size // WORLD_SIZE * 2, imgsz=imgsz,
                                                 iou_thres=0.65 if is_coco else 0.60, single_cls=single_cls,
                                                 verbose=True, save_json=is_coco, model=attempt_load(f, device).half(),
                                                 dataloader=val_loader, save_dir=save_dir, plots=plots,
                                                 callbacks=callbacks,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--optimizer', type=str, choices=['SGD', 'Adam', 'AdamW'], default='', help='optimizer')
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(opt.evolve):  # generations to evolve
            ...
            keys = ('metrics/precision', 'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.5:0.95', 'metrics/F1', 'val/box_loss',
